...
- `src/services/api.js`: cachear promesa de `loadBcchData` para que llamadas concurrentes compartan el mismo fetch.
- `src/services/api.js`: normalizar el payload una sola vez y reutilizar `bcchDataCache`.
- `src/services/api.js`: cachear resultados derivados (ej. `getChartData`) por `indicatorId`.
...
- `src/App.jsx`: mover constantes puras fuera del componente (p. ej. mapas de regiones y arrays de opciones).
- `src/App.jsx`: `useMemo` para datos derivados pesados (sideIndicators, charts, laborCards, ranges).
- `src/components/Overview/MacroMap.jsx`: `React.memo` si props no cambian frecuentemente.
- `src/components/Overview/MacroCard.jsx`: `React.memo` + evitar recalcular formatters en cada render.
- `src/components/Charts/TrendChart.jsx`: `React.memo` y memoizar calculos de dominio/average si `data` no cambia.
...
- `src/components/Overview/MacroMap.jsx`: eliminar `colorScale` sin uso.

## 4) CSS y fuentes
**Objetivo:** reducir bloqueos de render y duplicaciones.

- `src/App.jsx`: quitar import duplicado de `./styles/global.css` (ya se importa en `src/main.jsx`).
...
- `src/services/api.js`: asegurar que los fallbacks `/api/*` solo se usen en desarrollo.
